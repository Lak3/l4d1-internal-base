...
void CUtil_Math::BuildTransformedBox(Vector* v2, const Vector bbmin, const Vector bbmax, const matrix3x4_t& m, const float flRotate)
{
	Vector v[8];
...
float CUtil_Math::GetFovBetween(const Vector vSrc, const Vector vDst)
{
	...
	const float flResult = RAD2DEGF(::acosf(v_Dst.Dot(v_Src) / v_Dst.LenghtSqr()));

	return ::isfinite(flResult) ? flResult : FLT_MAX;
}
